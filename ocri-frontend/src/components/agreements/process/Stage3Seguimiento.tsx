...
function DeliverableCard({
    ...
    onEvaluate: (id: number, title: string, decision: 'APPROVED' | 'OBSERVED') => void;
}) {
        ...
        <div className="border border-gray-200 bg-white">
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-gray-800">{deliverable.title}</div>
                    {deliverable.period && (
                ...
                {deliverable.submitted_at && (
                    <span className="text-xs text-gray-500 hidden sm:inline">
                        Enviado: {new Date(deliverable.submitted_at).toLocaleDateString('es-PE')}
                    </span>
                )}
                {expanded ? (
            ...
            {expanded && (
                <div className="px-4 pb-4 space-y-3 border-t border-gray-100 pt-3">
                    {documents.length > 0 ? (
                        ...
                        <p className="text-sm text-gray-500">Aún no se ha presentado ningún archivo.</p>
                    )}
...
export default function Stage3Seguimiento({
    ...
    const handleSubmitFile = async (deliverable: Deliverable, file: File) => {
        try {
            ...
            const message = err instanceof Error ? err.message : 'Error al enviar entregable';
            toast.error(message);
        }
    };
    ...
    const handleRequestReport = async () => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : 'Error al solicitar informe';
            toast.error(message);
        } finally {
    ...
    const handleEvaluate = async () => {
        ...
        try {
            await evaluateDeliverable(evaluateModal.id, evaluateModal.decision, evaluateObs || undefined);
            toast.success(
            ...
            const message = err instanceof Error ? err.message : 'Error al evaluar entregable';
            toast.error(message);
        } finally {
    ...
    return (
        ...
        >
            ...
            {evaluateModal && (
                <ModalShell
                    title={evaluateModal.decision === 'APPROVED' ? 'Aprobar Entregable' : 'Observar Entregable'}
                    icon={evaluateModal.decision === 'APPROVED' ? CheckCircle2 : AlertTriangle}
                    footer={
                        <>
                            <button
                                onClick={() => {
