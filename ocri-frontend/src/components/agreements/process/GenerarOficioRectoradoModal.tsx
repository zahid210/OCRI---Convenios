...
import { getOficioRectoradoTemplate } from "@/lib/api";
...
export default function GenerarOficioRectoradoModal({
  agreementId,
  onGenerate,
  onCancel,
}: GenerarOficioRectoradoModalProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    getOficioRectoradoTemplate(agreementId)
      .then((data) => {
        if (cancelled) return;
      ...
      .catch(() => {
        ...
        toastRef.current.error(
          "No se pudo cargar la plantilla del oficio a Rectorado.",
        );
        onCancelRef.current();
      })
      .finally(() => {
  ...
  const handleSubmit = async () => {
    ...
    setIsSending(true);
    try {
      await onGenerate({ bodyHtml: oficioHtml, oficio_number: oficioNumber });
    } catch {
