...
export default function ReportsPage() {
    ...
    const [topInstitutions, setTopInstitutions] = useState<ReportInstitutionRow[]>([]);
        ...
        async function loadLookups() {
            try {
                const [countriesRes, typesRes, institutionsRes] = await Promise.all([
                    fetcher<string[]>('/institutions/countries').catch(() => [] as string[]),
                    fetcher<AgreementType[]>('/agreements/lookups/types').catch(() => [] as AgreementType[]),
                    fetcher<Institution[]>('/agreements/lookups/institutions').catch(() => [] as Institution[]),
                ]);
                if (isMounted) {
                    setCountries(Array.isArray(countriesRes) ? countriesRes : []);
                    setTypes(Array.isArray(typesRes) ? typesRes : []);
                    setInstitutions(Array.isArray(institutionsRes) ? institutionsRes : []);
                }
            } catch {
                if (isMounted) {
                    setCountries([]);
    ...
    useEffect(() => {
        ...
        async function loadReports() {
            try {
                ...
                if (activeFilters.typeId) params.set('agreement_type_id', activeFilters.typeId);
                if (activeFilters.institutionId) params.set('institution_id', activeFilters.institutionId);
                const query = params.toString();
                const suffix = query ? `?${query}` : '';

                const [summaryRes, byStatusRes, byCountryRes, byTypeRes, byInstitutionRes, topRes, expiringRes, expiredRes] =
                    await Promise.all([
    ...
    return (
        <div className="space-y-6 pb-12 font-sans text-gray-700">
            ...
            <form onSubmit={applyFilters} className="bg-white border border-gray-200 p-4 shadow-sm space-y-4">
                            ...
                            <div className="p-5 space-y-3">
                                {byStatus.map((row) => (
                                    <div key={row.estado} className="flex items-center gap-3">
                                        <span className="w-32 text-sm text-gray-700">{row.estado}</span>
                            ...
                            <div className="p-5 space-y-3">
                                {byCountry.map((row) => (
                                    <div key={row.pais} className="flex items-center gap-3">
                                        ...
                                        <span className="flex-1 text-sm text-gray-700">{row.pais}</span>
                                        <span className="text-sm font-semibold text-gray-800">{row.cantidad}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="p-5 space-y-3">
                                {byType.map((row) => (
                                    <div key={row.tipo} className="flex items-center gap-3">
                                        ...
                                        <span className="w-40 text-sm text-gray-700">{row.tipo}</span>
                                        <div className="flex-1 bg-gray-100 h-5">
                                            <div
                                                className="h-5 bg-[#094d37]"
                                                style={{ width: `${(row.cantidad / maxByType) * 100}%` }}
                                            />
                                        </div>
                                        <span className="w-10 text-right text-sm font-semibold text-gray-800">
                                            {row.cantidad}
                                ...
                                {byInstitution.length === 0 ? (
                                    ...
                                    byInstitution.map((row) => (
                                        <div key={row.institucion} className="flex items-center gap-3">
                                            ...
                                            <span className="flex-1 truncate text-sm text-gray-700" title={row.institucion}>
                                                {row.institucion}
                                            </span>
                                            <span className="text-sm font-semibold text-gray-800">{row.cantidad}</span>
                                        </div>
                                    ...
                                    {expiring.length === 0 ? (
                                        ...
                                        expiring.map((row) => (
                                            <tr key={row.id} className="hover:bg-gray-50">
                                                <td className="py-3 px-4 text-sm text-gray-800">{row.expediente}</td>
                                                <td className="py-3 px-4 text-sm text-gray-700">{row.institucion}</td>
