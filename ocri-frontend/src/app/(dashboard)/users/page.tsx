...
const EMPTY_FORM: UserForm = { name: '', email: '', password: '', role: 'viewer' };
...
export default function UsersPage() {
    ...
    useEffect(() => {
        ...
        async function loadUsers() {
            try {
                ...
                const res = await fetcher<PaginatedResponse<UserItem>>(`/users?${params.toString()}`);
        ...
        try {
            ...
            setData((prev) => {
                ...
                const newData = exists
                    ...
                    : [saved, ...prev.data];
                return { ...prev, data: newData, meta: { ...prev.meta, total: prev.meta.total + (exists ? 0 : 1) } };
            });
    ...
    const inputClass =
        'w-full px-3 py-2 text-sm bg-white border border-gray-300 focus:outline-none focus:border-[#df9f1f] text-gray-800';
                ...
                <div className="overflow-x-auto min-h-[300px]">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tr className="bg-[#f8f9fa] border-b border-gray-200">
                            <th className="py-4 pl-10 font-medium uppercase text-[11px] text-gray-600 tracking-wider">Usuario</th>
                            <th className="py-4 font-medium uppercase text-[11px] text-gray-600 tracking-wider">Correo</th>
                            <th className="py-4 font-medium uppercase text-[11px] text-gray-600 tracking-wider">Rol</th>
                            <th className="py-4 font-medium uppercase text-[11px] text-gray-600 tracking-wider text-center">Estado</th>
                            ...
                            data?.data.map((user) => {
                                ...
                                return (
                                    <tr key={user.id} className="group hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="py-4">
                                            <span className={`inline-flex items-center px-2.5 py-1 uppercase text-xs border ${roleBadgeColors[user.role] || 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                ...
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white border border-gray-200 w-full max-w-md p-6 shadow-xl space-y-4 relative">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                <label className="block text-xs font-semibold uppercase text-gray-600">
                                    Contraseña {editingUser ? '(dejar vacío para no cambiarla)' : <span className="text-red-500">*</span>}
                                </label>
                                <input
                                    type="password"
