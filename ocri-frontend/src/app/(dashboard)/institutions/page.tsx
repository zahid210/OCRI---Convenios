    ...
    useEffect(() => {
        ...
        async function loadInstitutions() {
            try {
                ...
                const res = await fetcher<InstitutionListResponse>(`/institutions?${params.toString()}`);

                if (isMounted) setData(res);
            } catch (err) {
                if (isMounted) {
                    console.error('Error al cargar instituciones:', err);
                    setError('Ocurrió un error al cargar el directorio de instituciones.');
                }
            } finally {
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        {loading ? (
                            ...
                            data?.data.map((institution) => (
                                <tr key={institution.id} className="group hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="py-5">
                                        <span className={`inline-flex items-center px-2.5 py-1 uppercase text-xs border ${
                                            typeColors[institution.type || ''] || 'bg-gray-50 text-gray-600 border-gray-200'
                                        }`}>
